...
csv_x_vtk_mesh=np.expand_dims(df['X [ m ]'].to_numpy(),1)
csv_y_vtk_mesh=np.expand_dims(df[' Y [ m ]'].to_numpy(),1)
csv_z_vtk_mesh=np.expand_dims(df[' Z [ m ]'].to_numpy(),1)
...
# for i in range(csv_nb_points):
# 	flo_arr = float(csv_xb_vtk_mesh[i])
...
# csv_n_points = len(df)
# print ('n_points of at wall' ,csv_n_pointsw)
...
#
# x_data[:,0] = csv_x_vtk_mesh[0:csv_N_pts_data,0]
# y_data[:,0] = csv_y_vtk_mesh[0:csv_N_pts_data,0]
# z_data[:,0] = csv_z_vtk_mesh[0:csv_N_pts_data,0]
#
# data_vel = np.array([df['f_17_0'].to_numpy(),df['f_17_1'].to_numpy(),df['f_17_2'].to_numpy()])
# points = np.array([x_data[:,0],y_data[:,0],z_data[:,0]])
# source_points =np.array([df['Points_0'].to_numpy(),df['Points_1'].to_numpy(),df['Points_2'].to_numpy()])
# # 使用 NumPy 创建无结构网格数据
#
#
#
...
data_vel = np.array([df[' Velocity u [ m s^-1 ]'].to_numpy(),df[' Velocity v [ m s^-1 ]'].to_numpy(),df[' Velocity w [ m s^-1 ]'].to_numpy()])
data_vel_u = data_vel[0,:] / U_scale
data_vel_v = data_vel[1,:] / U_scale
data_vel_w = data_vel[2,:] / U_scale
